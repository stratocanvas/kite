...
function IndoorMap({
	boothLocations,
	items,
}: { boothLocations: BoothLocation[]; items: any }) {
			...
			try {
				...
				img.onload = () => {
					...
					const projection = geoIdentity()
						...
						.fitSize(
							[width, height],
							feature(topojsonData, topojsonData.objects["event-map"]),
						);
					...
					pathGroup
						.selectAll("path")
						.data(
							feature(topojsonData, topojsonData.objects["event-map"]).features,
						)
						.enter()
						.append("path")
						.attr("d", pathGenerator)
						.attr("fill", (d) => {
							const location = filteredBoothLocations?.find(
								(loc) => loc.id === d.properties.id,
							);
							const boothExists = boothData.some((booth: any) =>
								booth.locations.includes(d.properties.id),
							);
							const completed = filteredBoothLocations.find(
								(loc) => loc.id === d.properties.id,
							)?.completed;
							if (completed) {
							...
							d3.active(this)
								...
								.attr("fill", function (d) {
									if (
										...
										boothData.some(
											(booth: any) =>
												booth.locations.includes(selectedBooth) &&
												booth.locations.includes(d.properties.id),
										)
									) {
										const location = filteredBoothLocations?.find(
					...
					pathGroup
						...
						.each(function (d) {
							...
							const inCart = filteredBoothLocations?.some(
								(loc) => loc.id === d.properties.id && loc.type === "cart",
							);
							if (inCart && view !== "wishlist") {
								...
								iconGroup.append("path").attr("d", "M3 6h18");

								iconGroup.append("path").attr("d", "M16 10a4 4 0 0 1-8 0");
							}
							const completed = filteredBoothLocations.find(
								(loc) => loc.id === d.properties.id,
							)?.completed;
							if (completed) {
								...
								d3.select(this)
									...
									.attr("transform", () => {
										return height > width
											? `rotate(90 ${pathGenerator.centroid(d)})`
											: "";
									});
							}
						});
				...
				<DropdownMenu>
					...
					<DropdownMenuContent>
						<DropdownMenuRadioGroup value={view} onValueChange={setView}>
							...
							<DropdownMenuRadioItem value="wishlist">
								위시리스트
							</DropdownMenuRadioItem>
							<DropdownMenuRadioItem value="cart">
								장바구니
							</DropdownMenuRadioItem>
						</DropdownMenuRadioGroup>
					</DropdownMenuContent>
					...
					<Carousel>
						<CarouselContent>
							{boothData
								...
								.map((booth: any) => {
									const isBoothInCart = items.some(
										(item) => item.boothId === booth.booth_id,
									);
									if (booth.locations.includes(selectedBooth)) {
										return (
											<CarouselItem key={booth.booth_id}>
												<Card className="bg-background/80 backdrop-blur-md">
													<CardHeader>
														...
														<Button
															...
															size="icon"
															variant="ghost"
															className="absolute top-4 right-4 rounded-full w-6 h-6 text-muted-foreground"
														...
														<CardDescription>
															{booth.locations.sort().length > 1
																? `${booth.locations[0]}-${
																		booth.locations[booth.locations.length - 1]
																  }`
																: booth.locations[0]}{" "}
															·{" "}
															{Array.isArray(booth?.date) &&
															booth?.date.length === 2
																? "양일"
