...
export default function BoothProfile({
	...
	const sortedLocations = useMemo(
		() =>
			data?.location?.sort((a: string, b: string) => a.localeCompare(b)) || [],
		[data?.locations],
	);
	const locationDisplay = useMemo(() => {
		...
		if (sortedLocations.length > 1) {
			return `${sortedLocations[0]}-${
				sortedLocations[sortedLocations.length - 1].match(/\d+$/)[0]
			}`;
		}
		return sortedLocations[0];
	}, [sortedLocations]);
	...
	return (
		<>
			<Card
				key={data?._id}
				className="h-auto border-none shadow-none relative xl:rounded-lg xl:h-[94vh]"
				ref={buttonRef}
			>
				<div className="relative">
					...
					>
						<div className="absolute inset-0 overflow-hidden xl:rounded-b-lg">
							...
							>
								{data?.thumbnail ? (
									...
									<div className="bg-muted flex justify-center items-center w-full h-full rounded-lg">
										<ImageOff />
									</div>
								)}
							</div>
						...
						<div className="absolute bottom-0 w-full h-2/3 md:h-1/3 rounded-b-lg">
							<div
								className="absolute top-0 left-0 w-full h-full xl:rounded-lg"
								style={{
			...
			<div
				className={`py-2 fixed z-40 top-0 left-1/2 transform -translate-x-1/2 bg-background/70 backdrop-blur-md w-full transition-all duration-300 ${
					buttonInView
