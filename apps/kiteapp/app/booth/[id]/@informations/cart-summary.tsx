...
export default function CartSummary({ boothId }: CartSummaryProps) {
		...
		mutate(
			async (currentItems) => {
				const optimisticItems =
					currentItems
						?.map((ci) =>
							ci.productId === item.productId && ci.optionId === item.optionId
								? updatedItem
								: ci,
						)
				...
				try {
					newQuantity > 0
						? await AddOrUpdateCart(item.productId, item.optionId, newQuantity)
						: await DeleteCart(item.productId, item.optionId);
	...
	*/};
	...
	const groupedItems = useMemo<GroupedItems>(
		() =>
			items?.reduce((acc: GroupedItems, item: Item) => {
				if (!acc[item.productId])
					acc[item.productId] = { productName: item.productName, options: [] };
				acc[item.productId].options.push(item);
				return acc;
			}, {}) || {},
		[items],
	);
							...
							<CardHeader>
								<div className="flex justify-between">
									...
									<Button
										...
										size="icon"
										variant="ghost"
										className="h-6 w-6 rounded-full backdrop-blur bg-black/30 dark:bg-white/30 hover:bg-black/20 dark:hover:bg-white/20"
									>
